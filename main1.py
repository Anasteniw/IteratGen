class FlatIterator:
    ...
    def __next__(self):
        self.inner_list_cursor +=1
        if self.inner_list_cursor is len(self.list_of_lists[self.outer_list_cursor]):
            self.outer_list_cursor += 1
            self.inner_list_cursor = 0
        if self.outer_list_cursor is len(self.list_of_lists):
            raise StopIteration
...
res =list(FlatIterator(list_of_lists_1))
print (res)
